...
def dbSave(romance):
    ...
    for obj in romance:
        ...
        try :
            ...
            if Romance_model.objects.filter(movie_id__iexact=movie_id).count() == 0:
                Romance_model(movie_id=movie_id,title=title,released_date=released_date,overview=overview,poster_path=poster_path,genres=genres).save()
                print("저장완료")
